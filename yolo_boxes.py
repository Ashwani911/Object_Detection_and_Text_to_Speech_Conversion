...
while True:
    ...
    if results:
        ...
        for box in boxes:
            ...
            class_values = [0] * len(class_list)
            class_values[clsID] = 1

            # Prepare values in the required format for display
            table_values = [
                ("Pc", round(conf * 100, 2)),    # Confidence
                ("Bx", Bx),                      # Center X
                ("By", By),                      # Center Y
                ("Bw", Bw),                      # Width
                ("Bh", Bh),                      # Height
                *[(f"C{i + 1}", class_values[i]) for i in range(len(class_values)) if class_values[i] == 1]
            ]

            print(f"Confidence (Pc): {table_values[0][1]}%, Bx: {Bx}, By: {By}, Bw: {Bw}, Bh: {Bh}, Classes: {class_values}")
